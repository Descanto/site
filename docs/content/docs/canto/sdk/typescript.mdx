...
desktop.patch({ idleTimeoutSecs? }) -> Promise<Desktop> // e.g. change TTL / disable auto-hibernate (0)
desktop.fork({ count, acknowledgeSharedState, ephemeral?, wait?, expectedGeneration? }) -> Promise<Desktop[]>
...
const status = await proc.status({ tailBytes? }); // -> ProcessStatusResponse
...
`status()` re-fetches `GET /v1/desktops/{id}/processes/{process_id}` each
...
`acknowledgeSharedState` are required, `ephemeral` optional:

...
<Callout title="Fork">
  ...
  desktop -- see [Concepts: Forking](/docs/canto/concepts/forking) for the
  full model.
</Callout>
...
- Set it at creation via `CreateDesktopBody.idle_timeout_secs`, or change it
  ...
  [`PATCH /v1/desktops/{id}`](/docs/canto/api/desktops#update-a-desktop):
...
<Callout title="Ephemeral desktops auto-destroy, not just hibernate" type="warn">
  ...
  [fork](/docs/canto/concepts/forking) with `"ephemeral": true`) does not

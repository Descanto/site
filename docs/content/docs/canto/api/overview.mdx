...
access token. See [Concepts: Orgs and auth](/docs/canto/concepts/orgs-and-auth)
...
| `404 Not Found` | The desktop/operation truly doesn't exist, **or** it belongs to a different org than the caller's -- these two cases are byte-identical on purpose. See [org scoping](/docs/canto/concepts/orgs-and-auth#org-scoping-404-never-403). |
...
[TypeScript SDK](/docs/canto/sdk/typescript#retry-semantics)'s default retry
...
- **`exec` timeouts** default to 60s and are clamped (never rejected) to a
  **300s** maximum, regardless of what `timeout_secs` requests. Ignored
  ...
  [exec and files](/docs/canto/api/exec-and-files#run-a-command-detached).

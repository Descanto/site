...
<Callout title="Private until launch" type="warn">
  ...
  `@descanto/vm-sdk` is bundled into `dist/index.js` at build time, so `npx`/`bunx
  @descanto/vm-mcp` will work correctly once this package is published.
</Callout>
...
Once published, `command`/`args` become `npx`/`bunx -y @descanto/vm-mcp` instead
of a local path.

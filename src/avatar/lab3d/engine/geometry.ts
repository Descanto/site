...
const densifyClosedPoints = (points: Point3[], maximumDistance = 7) => {
  let ring = points.flatMap((point, index) => {
  ...
  // Patch (bot): two passes of closed-ring Laplacian smoothing. Silhouette
  // extraction picks slightly jittery edge points off the sampled 3D surface;
  // without this the Catmull-Rom spline faithfully bends through every wobble
  // and large renders look hand-cut. Averaging each point with its neighbours
  // removes the jitter while preserving the overall outline.
